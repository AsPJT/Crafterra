...
namespace Crafterra {
	...
	namespace System {
		void crafterraMain(::Crafterra::Resource& resource_) {
			...
			::Crafterra::Enum::ActorDirection cdt = ::Crafterra::Enum::ActorDirection::down;
			...
			while (::Crafterra::System::Update()) {
				...
				{
					key.setKey();
					//if (key.isPressed(KEY_INPUT_LSHIFT)
					//	&& key.isPressed(KEY_INPUT_RSHIFT)
					//	&& key.isPressed(KEY_INPUT_LCONTROL)
					//	&& key.isPressed(KEY_INPUT_RCONTROL)
					//	&& key.isPressed(KEY_INPUT_LALT)
					//	&& key.isPressed(KEY_INPUT_RALT)
					//	&& key.isPressed(KEY_INPUT_LWIN)
					//	&& key.isPressed(KEY_INPUT_RWIN)
					//	)
					{
						if (key.isPressed(::AsLib2::Key::key_a) || key.isPressed(::AsLib2::Key::key_left)) {
							cs.camera_size.moveX(-key_displacement);
							cdt = ::Crafterra::Enum::ActorDirection::left;
						}
						if (key.isPressed(::AsLib2::Key::key_d) || key.isPressed(::AsLib2::Key::key_right)) {
							cs.camera_size.moveX(key_displacement);
							cdt = ::Crafterra::Enum::ActorDirection::right;
						}
						if (key.isPressed(::AsLib2::Key::key_w) || key.isPressed(::AsLib2::Key::key_up)) {
							cs.camera_size.moveY(-key_displacement);
							cdt = ::Crafterra::Enum::ActorDirection::up;
						}
						if (key.isPressed(::AsLib2::Key::key_s) || key.isPressed(::AsLib2::Key::key_down)) {
							cs.camera_size.moveY(key_displacement);
							cdt = ::Crafterra::Enum::ActorDirection::down;
						}
