...
namespace Crafterra {
	...
	void perlinNoise(
		...
		const ::Asc::DataType::Uint32 chunk_index_x_, const ::Asc::DataType::Uint32 chunk_index_y_, const Asc::DataType::IndexUint one_chunk_width_, const Asc::DataType::IndexUint one_chunk_height_,
		const Asc::DataType::IndexUint start_x_, const Asc::DataType::IndexUint start_y_, const Asc::DataType::IndexUint end_x_, const Asc::DataType::IndexUint end_y_,
		::dtl::utility::PerlinNoise<double>& perlin, const double frequency_, const Asc::DataType::IndexUint octaves_,
		const ElevationUint max_height_, const ElevationUint min_height_ = 0) {

		for (Asc::DataType::IndexUint row_index{ start_y_ }, row{}; row_index < end_y_; ++row_index, ++row)
			for (Asc::DataType::IndexUint col_index{ start_x_ }, col{}; col_index < end_x_; ++col_index, ++col)
				matrix_(col_index, row_index,
				min_height_ + static_cast<ElevationUint>(
				(max_height_ - min_height_) *
					perlin.octaveNoise(octaves_,
					(::Asc::DataType::Uint64(chunk_index_x_) * ::Asc::DataType::Uint64(one_chunk_width_) + ::Asc::DataType::Uint64(col)) / frequency_,
						((::Asc::DataType::Uint64(chunk_index_y_) * ::Asc::DataType::Uint64(one_chunk_height_) + ::Asc::DataType::Uint64(row)) / frequency_)
					)
					));
	}
	...
	class XorShift32 {
	private:
		::Asc::DataType::Uint32 seed;
	public:
		::Asc::DataType::Uint32 getRand() {
			seed ^= seed << 13;
			seed ^= seed >> 17;
			seed ^= seed << 5;
			return seed;
		...
		Asc::DataType::Int32 getProbabilityDivision(const double probability_, const Asc::DataType::Int32 division_) {
		...
		XorShift32(const ::Asc::DataType::Uint32 seed_) : seed(seed_) {}
	};
	...
	class Terrain {
		...
		// 暫定的なマップデータ
		using MapMat = ::Asc::DataType::Matrix<MapChip, init_field_map_width, init_field_map_height>;
		using shape_t = ElevationUint;

		::Asc::DataType::Uint32 temperature_seed;
		::Asc::DataType::Uint32 amount_of_rainfall_seed;
		::Asc::DataType::Uint32 elevation_seed;
	...
	public:
		...
		Terrain(const ::Asc::DataType::Uint32 temperature_seed_, const ::Asc::DataType::Uint32 amount_of_rainfall_seed_, const ::Asc::DataType::Uint32 elevation_seed_)
			:
		...
		void setTerrain(MapMat& field_map_matrix) const {
			...
			for (::Asc::DataType::IndexUint row{}; row < init_field_map_height; ++row)
				for (::Asc::DataType::IndexUint col{}; col < init_field_map_width; ++col) {
					MapChip& field_map = field_map_matrix[row][col];
					field_map.setElevation3(0); // 初期化

					ElevationUint elevation3 = 0;
					for (::Asc::DataType::IndexUint row3{ row }, block_index{}; elevation3 <= field_map.getBlockElevation() && block_index < 128; --row3, ++elevation3, ++block_index) {
					...
					const Asc::DataType::Int32 row2 = Asc::DataType::Int32(row) - Asc::DataType::Int32(field_map_matrix[row][col].getBlockElevation());
					if (row2 >= 0) {
			...
			// どこが崖になっているか調べる
			//for (::Asc::DataType::IndexUint col{}; col < init_field_map_width; ++col)
			//	for (::Asc::DataType::IndexUint row{ 1 }; row < init_field_map_height; ++row) {
			//			field_map_matrix[row][col].setIsCliff(field_map_matrix[row][col].getElevation3() < field_map_matrix[row - 1][col].getElevation3()); // 崖
			//	}
			// 崖のオートタイルを計算
			for (::Asc::DataType::IndexUint col{ 1 }; col < init_field_map_width - 1; ++col)
				for (::Asc::DataType::IndexUint row{}; row < init_field_map_height - 1; ++row) {
			...
			for (::Asc::DataType::IndexUint col{ 1 }; col < init_field_map_width - 1; ++col)
				for (::Asc::DataType::IndexUint row{ 1 }; row < init_field_map_height - 1; ++row) {
		...
		void perlinNoiseGeneration(MapMat& field_map_matrix, const ::Asc::DataType::Uint32 chunk_index_x_, const ::Asc::DataType::Uint32 chunk_index_y_, const ::Asc::DataType::Uint32 start_x_, const ::Asc::DataType::Uint32 start_y_, const ::Asc::DataType::Uint32 end_x_, const ::Asc::DataType::Uint32 end_y_) {
			//温度
			perlinNoise(
				[&field_map_matrix](const Asc::DataType::IndexUint x_, const Asc::DataType::IndexUint y_, const ElevationUint value_) { field_map_matrix[y_][x_].setTemperature(value_); },
			...
			//降水量
			perlinNoise(
				[&field_map_matrix](const Asc::DataType::IndexUint x_, const Asc::DataType::IndexUint y_, const ElevationUint value_) { field_map_matrix[y_][x_].setAmountOfRainfall(value_); },
				chunk_index_x_, chunk_index_y_, init_field_map_width / 2, init_field_map_height / 2,
				start_x_, start_y_, end_x_, end_y_,
			...
			//標高
			perlinNoise(
				[&field_map_matrix](const Asc::DataType::IndexUint x_, const Asc::DataType::IndexUint y_, const ElevationUint value_) { field_map_matrix[y_][x_].setElevation(value_); },
				chunk_index_x_, chunk_index_y_, init_field_map_width / 2, init_field_map_height / 2,
				start_x_, start_y_, end_x_, end_y_,
		...
		void moveUpTerrain(MapMat& field_map_matrix, const ::Asc::DataType::Uint32 field_height_half_) const {
			for (::Asc::DataType::IndexUint row{}; row < field_height_half_; ++row)
				for (::Asc::DataType::IndexUint col{}; col < init_field_map_width; ++col) {
		...
		void moveDownTerrain(MapMat& field_map_matrix, const ::Asc::DataType::Uint32 field_height_half_) const {
			for (::Asc::DataType::IndexUint row{}; row < field_height_half_; ++row)
				for (::Asc::DataType::IndexUint col{}; col < init_field_map_width; ++col) {
		...
		void moveLeftTerrain(MapMat& field_map_matrix, const ::Asc::DataType::Uint32 field_width_half_) const {
			for (::Asc::DataType::IndexUint row{}; row < init_field_map_height; ++row)
				for (::Asc::DataType::IndexUint col{}; col < field_width_half_; ++col) {
					MapChip& field_map_after = field_map_matrix[row][col];
		...
		void moveRightTerrain(MapMat& field_map_matrix, const ::Asc::DataType::Uint32 field_width_half_) const {
			for (::Asc::DataType::IndexUint row{}; row < init_field_map_height; ++row)
				for (::Asc::DataType::IndexUint col{}; col < field_width_half_; ++col) {
		...
		// フィールドマップを生成
		void generation(MapMat& field_map_matrix, const ::Asc::DataType::Uint32 chunk_index_x_, const ::Asc::DataType::Uint32 chunk_index_y_, const ::Asc::DataType::Uint32 start_x_, const ::Asc::DataType::Uint32 start_y_, const ::Asc::DataType::Uint32 end_x_, const ::Asc::DataType::Uint32 end_y_) {
			...
			//バイオームの分類分け
			for (::Asc::DataType::IndexUint row{ start_y_ }; row < end_y_; ++row)
				for (::Asc::DataType::IndexUint col{ start_x_ }; col < end_x_; ++col) {
					MapChip& field_map = field_map_matrix[row][col];
					...
					for (Asc::DataType::IndexUint i = 0; i < elevation / 2; ++i) {
						field_map.setBlock(Block::cliff, i);
					}
					for (Asc::DataType::IndexUint i = elevation / 2 + 1; i < 128; ++i) {
					...
					if (field_map.getBiome() == MapChipTypeBiome::sea) {
						...
						for (Asc::DataType::IndexUint i = elevation / 2; i <= sea_elevation / 2; ++i) {
		...
		void initialGeneration(MapMat& field_map_matrix_, const ::Asc::DataType::Uint32 chunk_index_x_, const ::Asc::DataType::Uint32 chunk_index_y_) {
